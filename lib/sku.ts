...
export function generateAutoSku(spec: ProductSpecInput): string {
  ...
  let rawMat = (spec.materialGrade || `${spec.material || ''} ${spec.grade || ''}`.trim() || '').trim();
  if (rawMat === '-') rawMat = '';
  let matCode = rawMat.replace(/\s+/g, '').toUpperCase();

  // 2. Thread (e.g. FT, HT)
  let rawThread = (spec.thread || spec.metric || '').trim();
  if (rawThread === '-') rawThread = '';
  let threadCode = rawThread.replace(/\s+/g, '').toUpperCase();

  // 3. Jenis Barang (e.g. Baut Mur Hex M.5-P0.80-K8 -> BMH-M.5-P0.80-K8)
  let rawItemType = (spec.itemType || '').trim();
  let itemCode = rawItemType
    .replace(/Baut Mur Hex/gi, 'BMH')
    .replace(/Baut Hex/gi, 'BH')
    .replace(/Mur Hex/gi, 'MH')
    .replace(/Ring Ver/gi, 'RV')
    .replace(/Ring Plat/gi, 'RP')
    .replace(/\s+/g, '-')
    .toUpperCase();

  // 4. Panjang (e.g. 8 MM -> 8MM)
  let rawLength = (spec.length || '').trim();
  let lengthCode = (rawLength && rawLength !== '-') ? rawLength.replace(/\s+/g, '').toUpperCase() : '';

  // 5. Finishing (e.g. HTM, PTH, KNG, UCP, HDG)
  let rawFinishing = (spec.finishing || '').trim();
  let finishingCode = (rawFinishing && rawFinishing !== '-') ? rawFinishing.replace(/\s+/g, '').toUpperCase() : '';
